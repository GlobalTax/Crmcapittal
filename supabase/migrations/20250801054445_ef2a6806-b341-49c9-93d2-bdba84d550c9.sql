-- Drop all existing lead-related views to recreate them cleanly
DROP VIEW IF EXISTS vw_leads_kpi CASCADE;
DROP VIEW IF EXISTS vw_dashboard_leads_kpi CASCADE;
DROP VIEW IF EXISTS vw_dashboard_leads_funnel CASCADE;
DROP VIEW IF EXISTS vw_leads_funnel CASCADE;

-- Create enhanced leads KPI view with trends and temporal analysis
CREATE VIEW vw_leads_kpi AS
WITH lead_stats AS (
  SELECT 
...
CREATE VIEW vw_leads_funnel AS
WITH stage_stats AS (
  SELECT 
    ps.id as stage_id,
