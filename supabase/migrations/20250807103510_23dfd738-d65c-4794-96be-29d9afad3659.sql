-- Fase 2: Permisos Granulares & Sharing (actualizada)
...
-- Agregar columnas a document_access_logs existente
ALTER TABLE public.document_access_logs 
ADD COLUMN IF NOT EXISTS share_id UUID REFERENCES public.document_shares(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS session_duration INTEGER,
ADD COLUMN IF NOT EXISTS access_type TEXT DEFAULT 'view' CHECK (access_type IN ('view', 'download', 'print', 'edit')),
