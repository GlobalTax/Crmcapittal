...
create policy if not exists "Public can insert valuation events"
  on public.valuation_events
  for insert
  to public
  with check (true);

-- Optional: restrict select to authenticated users only (no policy for anon)
create policy if not exists "Authenticated can select valuation events"
  on public.valuation_events
  for select
  to authenticated
  using (true);
