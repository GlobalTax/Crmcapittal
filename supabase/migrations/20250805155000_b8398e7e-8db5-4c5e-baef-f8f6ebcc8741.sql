...
BEGIN
    ...
    -- Update NULL created_by fields across all tables (excluding leads temporarily)
    ...
    -- Skip leads table due to trigger issue - will handle separately
    ...
    -- Handle other tables that exist
    UPDATE cases SET created_by = default_user_id WHERE created_by IS NULL AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cases');
    UPDATE commission_rules SET created_by = default_user_id WHERE created_by IS NULL AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'commission_rules');
    UPDATE commission_settings SET updated_by = default_user_id WHERE updated_by IS NULL AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'commission_settings');
    
    -- Log the security fix
    RAISE NOTICE 'Updated NULL created_by fields with default user ID: %', default_user_id;
END $$;

-- 2. Make critical user reference fields NOT NULL (excluding problematic tables)
