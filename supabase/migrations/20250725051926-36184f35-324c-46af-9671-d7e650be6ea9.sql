-- Primero, verificar si ya existe un pipeline de tipo MANDATE
DO $$
DECLARE
  mandate_pipeline_id UUID;
  mandate_pipeline_exists BOOLEAN := FALSE;
BEGIN
  -- Verificar si ya existe un pipeline MANDATE
  SELECT id INTO mandate_pipeline_id
  FROM public.pipelines
  WHERE type = 'MANDATE' AND is_active = true
  LIMIT 1;
  
  IF mandate_pipeline_id IS NOT NULL THEN
    mandate_pipeline_exists := TRUE;
    RAISE NOTICE 'Pipeline MANDATE ya existe con ID: %', mandate_pipeline_id;
  END IF;
  
  -- Solo crear si no existe
  IF NOT mandate_pipeline_exists THEN
    ...
    -- Obtener el ID del nuevo pipeline
    SELECT id INTO mandate_pipeline_id
    FROM public.pipelines
    WHERE type = 'MANDATE' AND is_active = true
    LIMIT 1;
    
    RAISE NOTICE 'Pipeline MANDATE creado con ID: %', mandate_pipeline_id;
  END IF;
  
  -- Crear etapas si el pipeline existe y no tiene etapas
  IF mandate_pipeline_id IS NOT NULL AND NOT EXISTS (
    ...
    RAISE NOTICE 'Etapas creadas para pipeline MANDATE';
  END IF;
END $$;