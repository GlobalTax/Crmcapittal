...
SELECT 
  -- Total leads activos
  COUNT(*) FILTER (WHERE status NOT IN ('DISQUALIFIED', 'CONVERTED')) as total_leads,
  
  -- Leads hot (prioridad alta/urgente)
  COUNT(*) FILTER (WHERE priority IN ('high', 'urgent') AND status NOT IN ('DISQUALIFIED', 'CONVERTED')) as hot_leads,
  ...
  COALESCE(SUM(CASE 
    WHEN status NOT IN ('DISQUALIFIED', 'CONVERTED') THEN 
...
CREATE OR REPLACE VIEW public.vw_leads_funnel AS
SELECT 
