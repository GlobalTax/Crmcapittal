...
WITH lead_stats AS (
  SELECT 
    ...
    SUM(CASE WHEN status = 'QUALIFIED' THEN budget_range::numeric ELSE 0 END) as pipeline_value,
    AVG(EXTRACT(DAYS FROM NOW() - created_at)) FILTER (WHERE status = 'QUALIFIED') as avg_time_to_qualify
  FROM leads
  WHERE created_at >= CURRENT_DATE - INTERVAL '1 year' -- Only last year for performance
),
...
WITH stage_stats AS (
  SELECT 
    ...
    AVG(l.score) FILTER (WHERE l.score IS NOT NULL) as avg_score,
