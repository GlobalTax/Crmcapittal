-- Fix SECURITY DEFINER issue in HubSpot views
-- Drop existing views that have SECURITY DEFINER behavior
DROP VIEW IF EXISTS hubspot_deals_with_details;
DROP VIEW IF EXISTS hubspot_contacts_with_company;
DROP VIEW IF EXISTS hubspot_companies_with_stats;
DROP VIEW IF EXISTS hubspot_deals;
DROP VIEW IF EXISTS hubspot_contacts;
DROP VIEW IF EXISTS hubspot_companies;

-- Recreate views without SECURITY DEFINER to respect RLS policies
CREATE VIEW hubspot_companies AS
SELECT 
  id,
...
CREATE VIEW hubspot_contacts AS
SELECT 
  id,
  external_id as hubspot_id,
  name,
...
-- Recreate composite views
CREATE VIEW hubspot_companies_with_stats AS
SELECT 
  hc.*,
...
CREATE VIEW hubspot_contacts_with_company AS
SELECT 
  hcon.*,
...
CREATE VIEW hubspot_deals_with_details AS
SELECT 
  hd.*,
