  ...
  const { data: leads = [], isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.error('Error in useLeadContacts query:', err);
        return [];
      }
    },
    retry: 2,
  ...
  const updateLeadMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: UpdateContactData }) => {
      try {
        ...
        console.error('Error in updateLeadMutation:', err);
        throw err;
      }
    },
    onSuccess: () => {
  ...
  const deleteLeadMutation = useMutation({
    mutationFn: async (id: string) => {
      try {
        ...
        console.error('Error in deleteLeadMutation:', err);
        throw err;
      }
    },
    onSuccess: () => {
  ...
  const convertLeadMutation = useMutation({
    mutationFn: async (leadId: string) => {
      try {
        ...
        console.error('Error in convertLeadMutation:', err);
        throw err;
      }
    },
    onSuccess: () => {
  ...
  const { data: lead, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.error('Error in useLeadContact query:', err);
        throw err;
      }
    },
    enabled: !!id,
