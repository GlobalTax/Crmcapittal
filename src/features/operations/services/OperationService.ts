import { BaseService, ServiceResponse, PaginationParams, FilterParams } from '@/shared/services/BaseService';
import { Operation } from '../types/Operation';
import { CreateOperationData, UpdateOperationData } from '../types/OperationData';
  ...
  static async getOperations(role?: string, filters?: OperationFilters & PaginationParams): Promise<ServiceResponse<DbOperation[]>> {
    try {
      ...
      if (filters?.min_revenue || filters?.max_revenue) {
        if (filters.min_revenue && filters.min_revenue > 0) {
          query = query.gte('revenue', filters.min_revenue);
        }
        if (filters.max_revenue && filters.max_revenue > 0) {
          query = query.lte('revenue', filters.max_revenue);
        }
      }
